...
describe('IssueEntity', () => {
  ...
  describe('moveToProject', () => {
    ...
    it('should throw NotFoundError when issue does not exist', async () => {
      ...
      await expect(
        entity.moveToProject('nonexistent', 'project-123')
    ...
    it('should throw NotFoundError when project does not exist', async () => {
      ...
      await expect(
        entity.moveToProject('issue-123', 'nonexistent')
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('assignToUser', () => {
    ...
    it('should throw NotFoundError when issue does not exist', async () => {
      ...
      await expect(
        entity.assignToUser('nonexistent', 'user-123')
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('changePriority', () => {
    ...
    it('should throw error for invalid priority', async () => {
      await expect(
        entity.changePriority('issue-123', 5)
      ).rejects.toThrow('Priority must be between 0 and 4');

      await expect(
        entity.changePriority('issue-123', -1)
      ).rejects.toThrow('Priority must be between 0 and 4');
    });
  });
  ...
  describe('createSubIssue', () => {
    ...
    it('should throw NotFoundError when parent issue does not exist', async () => {
      ...
      await expect(
        entity.createSubIssue('nonexistent', TestFactory.issueCreate())
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('getSubIssues', () => {
    it('should fetch sub-issues', async () => {
      const mockIssue = createMockIssue();
      const mockChildren = [
        createMockIssue({ id: 'child-1' }),
        createMockIssue({ id: 'child-2' }),
      ];
