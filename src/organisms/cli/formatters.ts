...
export const formatters = {
  ...
  label(label: IssueLabel, indent = 0): void {
    ...
    if (category) {
      ...
      console.log(prefix, chalk.white(label.name), label.color ? chalk.hex(label.color)('●') : '');
    }
    ...
    if (hierarchy) {
      ...
      categories.forEach((categoryLabels, category) => {
        ...
        categoryLabels.forEach((label) => {
          ...
          console.log('    ', chalk.white(value), label.color ? chalk.hex(label.color)('●') : '');
          if (label.description) {
  ...
  analytics(analytics: TeamAnalytics): void {
    ...
    console.log(
      chalk.gray('    Open:             '),
      chalk.yellow(analytics.metrics.openIssues.toString()),
    );
    console.log(
      chalk.gray('    In Progress:      '),
      chalk.blue(analytics.metrics.inProgressIssues.toString()),
    );
    console.log(
      chalk.gray('    Completed:        '),
      chalk.green(analytics.metrics.completedIssues.toString()),
    );
  ...
  velocity(velocities: number[], cycleLabels?: string[]): void {
    ...
    velocities.forEach((v, i) => {
      ...
      console.log(chalk.gray('   '), label.padEnd(10), chalk.blue(bar), chalk.dim(v.toString()));
    });
