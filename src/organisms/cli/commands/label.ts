...
export function createLabelCommand(): Command {
  const labels = new Command('labels').alias('l').description('Manage labels');
      ...
      try {
        ...
        if (result.failed.length > 0) {
          ...
          result.failed.forEach(({ item, error }: any) => {
            console.log(chalk.red('  ✗'), item.name, chalk.dim(`- ${error}`));
          });
        }
      } catch (error) {
  ...
  labels
    ...
    .action(async (query, options) => {
      ...
      try {
        ...
        const allLabels = options.team ? await api.listByTeam(options.team) : await api.list();

        const lowerQuery = query.toLowerCase();
        const labels = allLabels.filter((label) => label.name.toLowerCase().includes(lowerQuery));
      ...
      try {
        ...
        if (!options.confirm) {
          ...
          const confirmed = await new Promise<boolean>((resolve) => {
            rl.question(
              chalk.yellow(`Are you sure you want to delete "${name}"? (y/N) `),
              (answer) => {
                rl.close();
