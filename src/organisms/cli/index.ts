    ...
    try {
      ...
      if (comments.nodes.length === 0) {
        ...
        for (const comment of comments.nodes) {
          ...
          console.log(
            chalk.yellow(`  ${user?.name || 'Unknown'} • ${createdAt.toLocaleDateString()}`),
          );
...
program
  ...
  .action(async (targetState, identifiers, options) => {
    ...
    try {
      ...
      const activeTeams = mergedConfig.teamFilter;
      const teamKey =
        options.team || (activeTeams && activeTeams.length > 0 ? activeTeams[0] : 'TASK');
      ...
      const targetWorkflowState = states.find(
        (s) =>
