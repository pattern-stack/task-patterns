...
export const SprintCalculations = {
   ...
   */
  velocity: (issues: SprintIssueData[]): number => {
    return issues
   ...
   */
  progress: (issues: SprintIssueData[]): number => {
    if (issues.length === 0) {
      return 0;
    }
  ...
  workloadDistribution: (
    issues: SprintIssueData[],
  ): Map<string, { count: number; points: number }> => {
   ...
   */
  averageCycleTime: (completedIssues: SprintIssueData[]): number => {
