...
export const IssueCalculations = {
  ...
  leadTime: (issue: IssueData): number | null => {
    if (!issue.createdAt || !issue.completedAt) return null;
  ...
  complexityScore: (issue: IssueData): number => {
    ...
    if (issue.priority && issue.priority <= 2) score += 20;
    
    // Add for dependencies
    if (issue.children?.length) score += issue.children.length * 5;
  ...
  slaStatus: (
    ...
    if (issue.stateType === 'completed') {
      ...
      status = 'breached';
    } else if (remainingHours < slaHours * 0.2) { // Less than 20% time remaining
      status = 'at-risk';
    } else {
      status = 'on-time';
    }
  ...
  estimatedCompletion: (
    issue: IssueData,
    teamVelocityPerDay: number
  ): Date | null => {
    if (!issue.estimate || issue.stateType === 'completed') return null;
